...
select
	 distinct customer
	 ,first_value(sales) over (partition by customer order by case when sales is not null then dated_on else '1900-01-01'::DATE end desc ) as sales
	,first_value(orders) over (partition by customer order by case when orders is not null then dated_on else '1900-01-01'::DATE end desc ) as orders
	,first_value(purchase) over (partition by customer order by case when purchase is not null then dated_on else '1900-01-01'::DATE end desc ) as purchase
	,first_value(expense) over (partition by customer order by case when expense is not null then dated_on else '1900-01-01'::DATE end desc) as expense
