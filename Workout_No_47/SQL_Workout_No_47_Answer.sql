...
select
	pp.id
	,pp.project_name
	,sum(fr.investment_amount) as _total_amount
from
	schoolofit.project_proposal pp
inner join
	schoolofit.fund_rising fr
		on fr.project_id = pp.id
group by
	pp.id,pp.project_name,pp.minimum_fund
having
	sum(fr.investment_amount) > pp.minimum_fund  * 1.0 * 1.50;
/