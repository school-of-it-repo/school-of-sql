...
select
	...
	,amount
	,order_no
	,case when amount>=400  and order_no >= 3 then 1 else 0 end as _bucket
from
	schoolofit.order_rank
)
select
	...
	,amount
	,order_no
	,case when 1= sum(_bucket) over(partition by id order by order_no ) then 'Y' else '' end as flag
from
	_data